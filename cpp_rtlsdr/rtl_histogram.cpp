/*
HOW TO COMPILE: g++ -o rtl_histogram rtl_histogram.cpp -lrtlsdr -std=c++17
*/
...
void calHistogram(const std::vector<uint8_t>& data) {

    // Count occurrences of each value
    std::vector<uint32_t> frequency(256, 0);
    for (uint32_t i = 0; i < data.size(); ++i) {
        ++frequency[data[i]];
    }

    // Calculate lower and upper bounds & max
    int lower_idx =0, upper_idx = 0, max_idx = 0;
    for (int value = 0; value <= 255; ++value) {
        if (frequency[value]) {lower_idx = value; break; }
    }

    for (int value = 255; value >= 0; --value) {
        if (frequency[value])   {upper_idx = value; break;}
    }

    for (int value = 0; value <= 255; ++value) {
        if (frequency[value] > frequency[max_idx]) max_idx = value; 
    }

    std::cout << "Histogram Max: " << std::setw(3) << max_idx << " (" << std::setw(5) <<  frequency[max_idx] << ")" 
    << " Lower: " << std::setw(3) << lower_idx << " (" << std::setw(5) <<  frequency[lower_idx] << ")"
    << " Upper: " << std::setw(3) << upper_idx << " (" << std::setw(5) <<  frequency[upper_idx] << ")" << std::endl;
...
int main() {
    try {
        ...
        dev0.setFrequency(391250000);  // Set frequency to 391 MHz
        dev0.setSampleRate(2560000);  // Set sample rate to 2.56 MSPS
        dev0.setGainMode(1);          // Set MANUAL gain mode
        dev0.setGain(0);

        // Read 16k samples from the device
        std::vector<uint8_t> buffer(16384);  // 16k sample buffer
		int res = dev0.resetBuffer();

    	const int maxIterations = 50;

    	for (int count = 0; count < maxIterations; ++count) {
        	dev0.setGain(count*10);
        	int bytesRead = dev0.readSamples(buffer);
        	std::cout << "Itr " << std::setw(3) << count + 1 << ": " << bytesRead << " bytes.  ";
            calHistogram(buffer);
        }
        dev0.close();
