...
void processBlock(const std::vector<uint8_t>& block, size_t blockSize, size_t subBlockSize, fftw_plan& fft_plan, 
    std::vector<std::complex<double>>& complexArray, std::vector<std::complex<double>>& fftOutput) {
...
void iterateAndProcessFolder(const std::string& folderPath, size_t subBlockSize, 
    fftw_plan& fft_plan, std::vector<std::complex<double>>& complexArray, std::vector<std::complex<double>>& fftOutput) {
        ...
        if (entry.is_regular_file()) {
            ...
            if (fileSize == superBlockSize) {
                ...
                for (size_t idx=1; idx <= superBlockCount; ++idx) {
                    ...
                    // Process the block
                    processBlock(block, blockSize, subBlockSize, fft_plan, complexArray, fftOutput);
...
int main() {
    ...
    try {
        iterateAndProcessFolder(folderPath, subBlockSize, plan, complexArray, fftOutput);
